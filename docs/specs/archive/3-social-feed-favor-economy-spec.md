...
> **Status:** ✅ **LAUNCH SCOPE COMPLETED (2026-06-22)** — §3–6 shipped; §7 Co-Focus Rooms remains deferred (post-launch, NOT built). · **Type:** extension of the existing feed (`src/app/(dashboard)/home`, `api/posts`)
>
> **What shipped:** schema in `db-sync.ts` (posts `type`/`skill_tag`/`accepted_answer_id`/`resolved_at` + `idx_type_resolved` + `helpful_events`); service `createPost` type-aware + `acceptAnswer`/`resolveAsk` + open-ask time-decayed ranking + `filter=help`/`help_matches`; throttled `new_help_request` + `answer_accepted` notifications; composer Status·Ask·Win switch + skill field; per-type post card (ask/win badges, win confetti, accepted-answer pin) on feed + permalink (owner mark-helpful/resolve); Win prompt over SSE (`win_prompt`, transient) seeding the composer. Schema columns went to `db-sync.ts` (Phase 3 consolidation) rather than in-route boot ALTERs. Tests: `posts.test.ts` covers accept/resolve guards + the credit-bridge log + recipient skill-filter/throttle. **Fast-follow:** real-DB integration test of the accept/notify SQL.
