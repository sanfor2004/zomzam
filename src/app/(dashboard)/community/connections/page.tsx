...
// ──────────────────────────────────────────────────────────
// DEVELOPMENT NAVIGATOR: GROUPED-LIST SECTION
// Calm gray group header (label + optional green count pill) over an inset,
// hairline-divided card — the iOS grouped-list rhythm shared with Settings.
// No overflow-hidden: it would clip a row's ••• context-menu popover.
// ──────────────────────────────────────────────────────────
    ...
    <div ref={pageRef} className="max-w-2xl mx-auto pb-20">
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          {active.length > 0 && (
            <Group title="Active now" count={activeCount}>
              {active.map((f) => (
                <ConnectionRow
                  key={f.id}
                  user={f}
