  ...
  if (loading) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center gap-3">
        ...
        <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Opening Projects Hub...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 dark:border-slate-800/60 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-550 dark:text-slate-400 mt-1 font-medium">Track active delivery pipelines, development milestones, and contract fulfillment.</p>
        </div>
        ...
        <div className="flex items-center gap-2.5 px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-150 dark:border-white/5 text-xs font-bold text-slate-500 dark:text-slate-400">
          Active Client Deliveries: <span className="text-slate-900 dark:text-white ml-0.5 font-extrabold">{projects.length}</span>
        </div>
      </div>

      {projects.length === 0 ? (
        <div className="bg-white dark:bg-[#1A1D24] border border-slate-100 dark:border-slate-800/60 p-16 rounded-3xl text-center text-slate-400 dark:text-slate-500 italic font-semibold text-xs shadow-apple">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {projects.map((project) => {
            ...
            return (
              <Card key={project.id} className="bg-white dark:bg-[#1A1D24] border border-slate-100 dark:border-slate-800/60 p-6 rounded-3xl relative overflow-hidden flex flex-col justify-between hover:border-[#EE5712]/30 transition-all duration-300 shadow-apple">
                <div className="space-y-4">
                  {/* Top Header */}
                  <div className="flex justify-between items-start gap-4">
                    <div className="min-w-0">
                      <h3 className="text-sm font-black text-slate-800 dark:text-white leading-tight truncate">{project.name}</h3>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-semibold">Client: {project.lead_company || project.lead_name}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/35 border border-slate-100 dark:border-slate-800/50 text-xs font-semibold">
                    <div className="space-y-0.5">
                      <span className="text-[9px] text-slate-400 dark:text-slate-550 font-bold uppercase tracking-wider block">Contract Value</span>
                      <div className="flex items-center text-slate-800 dark:text-white font-extrabold text-xs">
                        <DollarSign className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-450 shrink-0" />
                        <span>{parseFloat(project.amount).toLocaleString()} {project.currency}</span>
                      </div>
                    </div>
                    <div className="space-y-0.5">
                      <span className="text-[9px] text-slate-400 dark:text-slate-550 font-bold uppercase tracking-wider block">Started Date</span>
                ...
                {/* Footer Controls */}
                <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800/60 flex gap-2">
                  <a 
                    href="/time/tasks"
                    className="flex-1 text-xs font-bold h-9 bg-slate-50 hover:bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-800 dark:text-white rounded-xl flex items-center justify-center gap-1.5 transition-all text-center flex items-center justify-center cursor-pointer shadow-sm"
                  >
