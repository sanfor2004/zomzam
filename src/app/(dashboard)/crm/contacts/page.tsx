  ...
  if (loading) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center gap-3">
        ...
        <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Opening Client Directory...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 dark:border-slate-800/60 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">Directory of qualified customers, partners, and active accounts.</p>
        </div>
        ...
        <div className="flex items-center gap-2.5 px-3.5 py-2 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-150 dark:border-white/5 text-xs font-bold text-slate-500 dark:text-slate-400">
          Total Qualified Contacts: <span className="text-slate-900 dark:text-white ml-0.5 font-extrabold">{contacts.length}</span>
        </div>
      </div>
      ...
      {filteredContacts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredContacts.map((contact) => (
            <Card key={contact.id} className="bg-white dark:bg-[#1A1D24] border border-slate-100 dark:border-slate-800/60 p-6 rounded-3xl relative overflow-hidden flex flex-col justify-between hover:border-[#EE5712]/30 transition-all duration-300 shadow-apple">
              <div className="space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <h3 className="text-sm font-black text-slate-800 dark:text-white mt-2 leading-tight truncate">{contact.name}</h3>
                ...
                <div className="space-y-2 border-t border-slate-100 dark:border-slate-800/60 pt-3">
                  ...
                  {contact.address && (
                    <div className="flex items-start gap-2 text-xs text-slate-500 dark:text-slate-400 leading-snug">
              ...
              <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800/60 flex gap-2">
                <Button
                  ...
                  className="flex-grow text-xs font-semibold h-8 bg-slate-50 hover:bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-850 dark:text-white rounded-xl flex items-center justify-center gap-1 cursor-pointer shadow-sm transition-all"
                >
