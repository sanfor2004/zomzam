...
// Short-lived — only needs to survive the round trip to Google's consent
...
export const GET = withError(async (request) => {
  ...
  // The callback compares this against Google's returned `state`, so a
  // forged ?code= from an attacker-initiated flow can't complete a login.
  response.cookies.set('GOOGLE_OAUTH_STATE', state, oauthCookieOpts);
  response.cookies.set('GOOGLE_OAUTH_REDIRECT', redirectTo, oauthCookieOpts);
  return response;
});
