// LiquidEther — WebGL fluid simulation (React Bits)
// Adapted for Zomzam dashboard background use.
'use client';
  ...
  useEffect(() => {
    ...
    class CommonClass {
      constructor() {
        ...
        this.container = null; this.renderer = null; this.clockLastTime = 0;
      }
      init(container) {
        ...
        this.clockLastTime = typeof performance !== 'undefined' ? performance.now() : Date.now();
      }
      resize() {
      ...
      update() {
        ...
        this.delta = Math.max(0, (now - this.clockLastTime) / 1000);
    ...
    class Advection extends ShaderPass {
      constructor(simProps) {
        super({
          material: { vertexShader: face_vert, fragmentShader: advection_frag, uniforms: { boundarySpace: { value: simProps.cellScale }, px: { value: simProps.cellScale }, fboSize: { value: simProps.fboSize }, velocity: { value: simProps.src.texture }, dt: { value: simProps.dt }, isBFECC: { value: true } } },
          output: simProps.dst
        });
        this.uniforms = this.props.material.uniforms;
        this.init();
    ...
    class Output {
      ...
      init() {
        ...
        this.output = new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({ vertexShader: face_vert, fragmentShader: color_frag, transparent: true, depthWrite: false, uniforms: { velocity: { value: this.simulation.fbos.vel_0.texture }, boundarySpace: { value: new THREE.Vector2() }, palette: { value: paletteTex }, bgColor: { value: bgVec4 } } })
        );
