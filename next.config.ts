...
const nextConfig: NextConfig = {
  ...
  images: {
    ...
    // configured". OAuth sign-in (Google/Facebook) stores the provider's
    // profile-picture URL verbatim in `users.avatar`, so their photo CDNs need
    // the same allowance. Add other avatar hosts here if the upload pipeline
    // ever stores remote URLs from a new provider.
    remotePatterns: [
